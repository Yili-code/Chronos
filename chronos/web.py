...
async function loadProjects(){const xs=await api('/api/projects');document.querySelector('#projects').innerHTML=xs.length?xs.map(x=>`<div class="item"><div class="row"><strong>${esc(x.name)}</strong><span class="tag">${esc(x.branch)} · ${x.dirty?x.change_count+' 項未 commit':'乾淨'}</span></div><div class="muted">${esc(x.last_commit)}</div></div>`).join(''):'<div class="muted">找不到可追蹤的 Git repository。</div>'}
